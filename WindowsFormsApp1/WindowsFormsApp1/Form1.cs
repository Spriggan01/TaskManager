...
namespace TaskManager
{
    public partial class Form1 : Form
    {
        ...
        {
            string path = Interaction.InputBox("Enter name of program", "Running new task");
            try
            {
                Process.Start(path);
            }
        ...
        {
            GetProcesses();
            List<Process> filterdprocesses = processes.Where((x) => x.ProcessName.ToLower().Contains(toolStripTextBox1.Text.ToLower())).ToList<Process>();
            RefreshProcessesList(filterdprocesses, toolStripTextBox1.Text);
        }
